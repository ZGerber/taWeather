...
def plot_night(local_weat_list: List[int], remote_weat_list: List[int], sstart: np.ndarray, sstop: np.ndarray, smid: np.ndarray) -> None:
    ...
    plt.scatter(local_weat_list, np.zeros_like(local_weat_list), color='green', marker="*", s=50)
    plt.scatter(remote_weat_list, np.zeros_like(remote_weat_list), color='red', marker="*", s=50)
